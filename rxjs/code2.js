...
function getRequest(url) {
  ...
  return new Observable(sub => {
    request(url, { json: true }).then(rep => {sub.next(rep);sub.complete;}).catch(err => { sub.error(err) })
  })
}
const subscriber = getRequest('https://wxtestapplet.gac-nio.com/community/recommend/front/v3.1.4/list?current=1&recommendListSize=0&size=15')
.subscribe({
  next: val => {
      console.log(val)
  },
